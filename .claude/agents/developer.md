...
UI コンポーネント（ボタン・フォーム・モーダル・タブ・トースト等）の新規実装・既存改修を行う前に、必ず `ui-ux-pro-max` スキルを呼び出してベストプラクティスを取得すること。スキルから得られる以下の観点を実装に反映する:

- アクセシビリティ（コントラスト 4.5:1、フォーカスリング、`aria-*`、キーボード操作）
- タッチターゲット（最低 44×44px、隣接要素との 8px 以上の間隔）
- フォーム UX（可視ラベル、エラーはフィールド近傍、`inline-validation` は blur 時、touched ステート管理）
- アニメーション（150〜300ms、`transform`/`opacity` のみ、`prefers-reduced-motion` 対応）
- 状態の明確化（hover / pressed / disabled / focused を視覚的に区別）

呼び出し例:
```
/ui-ux-pro-max:ui-ux-pro-max （実装対象の UI 要素・ユーザーアクション・既存制約を渡す）
```

スキルの推奨内容と仕様書・既存サイトデザインシステムに齟齬がある場合は、仕様書 > 既存サイト > スキル推奨 の優先順位で判断し、判断理由を実装サマリの「開発者の判断事項」に明記する。
