...
仕様書の「UI・UX要件」「画面構成」「操作フロー」「フィードバック」セクションを記述する前に、必ず `ui-ux-pro-max` スキルを呼び出して設計指針を取得すること。スキルから得られる以下の観点を仕様書に反映する:

- 想定する製品タイプに合致するスタイル・カラーパレット・タイポグラフィの方向性
- 該当する UX ガイドライン（アクセシビリティ / タッチターゲット / フォーム / フィードバック / 動き等）の遵守要件
- アンチパターン（避けるべき設計）を「スコープ外」または「禁止事項」として明示

呼び出し例:
```
/ui-ux-pro-max:ui-ux-pro-max （対象ツールの製品タイプ・想定ユーザー・スタイルキーワードを渡す）
```

スキルの推奨内容と既存サイトのデザインシステム（`common/style.css` のカスタムプロパティ）に齟齬がある場合は、既存サイト側を優先し、その判断理由を仕様書の「UI・UX要件」末尾に注記する。
